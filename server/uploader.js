  ...
  return new Promise((resolve, reject) => {
    ...
    new IncomingForm().parse(request)
      .on('fileBegin', (name, file) => {
        uploadFile(file)
        if (name.match(/[\w\d]+{[\w\d]+}/i)) {
          const sanitizedName = name.replace(/{[\w\d]+}/i,"");
          const fileName = name.replace(/[\w\d]+{|}/ig, "");

          if (typeof data[sanitizedName] === "undefined") data[sanitizedName] = {};

          data[sanitizedName][fileName] = file.path.split(`${process.cwd()}/`).pop();
        } else {
